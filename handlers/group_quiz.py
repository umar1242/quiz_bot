...
# in-memory хранилища
_timers:            dict[int, asyncio.Task] = {}   # session_id → Task
_poll_to_session:   dict[str, int]          = {}   # poll_id    → session_id
_session_poll_idx:  dict[int, int]          = {}   # session_id → текущий idx
_session_chat:      dict[int, int]          = {}   # session_id → chat_id
_session_participants: dict[int, int]       = {}   # session_id → кол-во участников


# ─── Хелпер отмены таймера ───────────────────────────────────────────────────
...
async def _timer_task(bot: Bot, chat_id: int, session_id: int,
    ...
    """Ждёт seconds секунд, потом переходит к следующему вопросу."""
    try:
        await asyncio.sleep(int(seconds))

        # Проверяем по памяти — вдруг уже перешли досрочно (все ответили)
        if _session_poll_idx.get(session_id) != idx:
            log.info(f"_timer_task: session {session_id} idx {idx} — уже перешли, пропускаем")
            return

        # Проверяем что сессия ещё активна
        session = await get_active_group_session(chat_id)
        if not session or session.get("id") != session_id or not session.get("is_active"):
            log.info(f"_timer_task: session {session_id} — сессия неактивна, пропускаем")
            return

        log.info(f"_timer_task: session {session_id} idx {idx} — таймер истёк, переходим")
        await _advance_question(bot, chat_id, session_id, questions, idx)

    except asyncio.CancelledError:
        pass  # Таймер штатно отменён через _cancel_timer
    except Exception as e:
        log.exception(f"⚠️ Xatolik _timer_task: {e}")
...
async def _advance_question(bot: Bot, chat_id: int, session_id: int,
    ...
    try:
        log.info(f"_advance_question: session {session_id} idx {idx} — начало")

        session = await get_active_group_session(chat_id)
        if not session:
            log.warning(f"_advance_question: session {session_id} — сессия не найдена в БД")
            return
        if not session.get("is_active"):
            log.warning(f"_advance_question: session {session_id} — сессия неактивна")
            return

        # _send_question_result убран — quiz-тип сам показывает ✅/❌
        await asyncio.sleep(3)

        participants = _session_participants.get(session_id, 999)
        log.info(f"_advance_question: session {session_id} — переходим к вопросу {idx + 1}, участников: {participants}")
        await _send_question(bot, chat_id, session_id, questions, idx + 1, participants)

    except Exception as e:
        log.exception(f"⚠️ Xatolik _advance_question: {e}")
...
async def _send_question(bot: Bot, chat_id: int, session_id: int,
    ...
    log.info(f"_send_question: session {session_id} idx {idx} — вызвана")

    session = await get_active_group_session(chat_id)
    if not session:
        log.warning(f"_send_question: session {session_id} — сессия не найдена в БД")
        return
    if not session.get("is_active"):
        log.warning(f"_send_question: session {session_id} — сессия неактивна (is_active=False)")
        return

    total = len(session["questions"])
    log.info(f"_send_question: session {session_id} idx {idx}/{total}")

    if idx >= total:
        log.info(f"_send_question: session {session_id} — все вопросы закончились, финиш")
    ...
    # Quiz-тип: Telegram сам показывает ✅/❌ сразу после нажатия
    ...
    poll_msg = await bot.send_poll(
        ...
        is_anonymous       = False,
        open_period        = int(session["timer_seconds"]),
    )
...
# ─── Обработка ответов на quiz-poll ─────────────────────────────────────────
# При type="quiz" Telegram присылает poll_answer (анонимный=False обязателен)
...
async def group_poll_answer(poll_answer: PollAnswer, bot: Bot):
    ...
    if q_idx is None or not chat_id:
        return
    ...
    # Если все ответили — переходим досрочно
...
async def send_group_launch_card(message: Message, bot: Bot,
    ...
    safe_cat = re.sub(r'[^a-zA-Z0-9\u0400-\u04FF]', '_', category)
...
async def cmd_startquiz(message: Message, bot: Bot):
    ...
    # Считаем участников группы
    try:
        member_count      = await bot.get_chat_member_count(message.chat.id)
        participant_count = max(member_count - 1, 1)  # вычитаем бота
    except Exception:
...
async def cmd_stopquiz(message: Message, bot: Bot):
    ...
    # Чистим память
    sid = session["id"]
    _session_chat.pop(sid, None)
    _session_participants.pop(sid, None)
    _session_poll_idx.pop(sid, None)

    await message.answer("🛑 Test to'xtatildi.")
    await _send_scoreboard(bot, message.chat.id, sid,
